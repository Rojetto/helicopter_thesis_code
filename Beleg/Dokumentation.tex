...
%	Mit $J_i$ .. Trägheitsmoment um Achse $i$; $V_s$ .. Summe der Rotorspannung $V_f + V_b$; $V_d$ .. Differenz der Rotorspannungen $V_f - V_b$; $L_i$ .. Längenkonstante; $\mu_i$ .. Gleitreibungskoeffizient um Gelenk $i$. Dieses einfache Modell beinhaltet bereits Zentripetalkräfte sowie Gleitreibung der Gelenke.
	...
	Die beispielhafte Parametrierung in \ref{tab:pid_parameters} ist in der Lage, Anfangsfehler von ca. \ang{30} in den Winkeln, sowie kleine Störmomente auszugleichen.
	...
	\end{equation}
	zuerst analytisch linearisiert und am Arbeitspunkt ausgewertet, sodass man die Systemmatrizen
	\begin{subequations}
		\begin{align}
			\vect{A} &= \left. \partiell{\vect f}{\vect x}\right\vert_\mathrm{AP}\\
	...
	erhält.
	\begin{enumerate}
		\item Vollständiges Modell wird analytisch linearisiert
		\item Auswertung der Systemmatrizen an gewünschtem Arbeitspunkt
		\item ermöglicht Entwurf einer konstanden Zustandsrückführung
		\item Zwei Varianten implementiert: Polplatzierung und LQR
		\item LQR Prinzip und Parameterwerte
	\end{enumerate}
	\subsection{Folgeregelung}
	\subsubsection{Flachheitsbasierter Ansatz}
	\begin{enumerate}
		\item Herleitung Flachheit an allereinfachstem Modell
		\item kurz nennen, welche Modelle noch flach sind
		\item flache Regelungsform
		\item stabilisierende Dingsbums
		\item Problem der Ableitungen
	\end{enumerate}
	\subsubsection{Partielle Linearisierung durch Rückführung}
	\begin{enumerate}
		\item Modellgleichungen für die das geht
		\item Prinzip erklären
	\end{enumerate}
	
	\newpage % um zu überprüfen, ob ich schon 2 Seiten geschrieben habe

...
\end{subequations}
    ...
    %
	Eine Ruhelage in diesem System wird beschrieben durch zwei frei wählbare Winkel \(\varepsilon^* \in (-\frac{\pi}{2}, \frac{\pi}{2})\) und \(\lambda^* \in (-\pi, \pi)\), außerdem gilt \(\varphi^*=0\).
...
\label{eq:flat3}
	...
	Nun ist es leicht möglich, für das lineare System
	\begin{subequations}
		\begin{align}
			\ddot{\varepsilon} &= v_1\\
			\ddot{\lambda} &= v_2

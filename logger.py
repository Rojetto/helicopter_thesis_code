...
def plotObserver(bundle):
    ...
    plt.plot(ts, rad_squared2def_squared(cov_matrix[:, 0, 0]), label="var(p) (°^2)")
    plt.plot(ts, rad_squared2def_squared(cov_matrix[:, 1, 1]), label="var(e) (°^2)")
    plt.plot(ts, rad_squared2def_squared(cov_matrix[:, 2, 2]), label="var(lambda) (°^2)")
    plt.xlabel("Time (s)")
    plt.ylabel("Variance Angles")
    plt.legend()

    fig = custom_figure("Covariance Matrix (var(dp), var(de), var(dlambda))")
    plt.plot(ts, rad_squared2def_squared(cov_matrix[:, 3, 3]), label="var(dp) ((°/s)^2)")
    plt.plot(ts, rad_squared2def_squared(cov_matrix[:, 4, 4]), label="var(de) ((°/s)^2)")
    plt.plot(ts, rad_squared2def_squared(cov_matrix[:, 5, 5]), label="var(dlambda) ((°/s)^2)")
    plt.xlabel("Time (s)")
    plt.ylabel("Variance Angle Velocities")
    plt.legend()

    fig = custom_figure("Covariance Matrix (var(f), var(b))")
    plt.plot(ts, cov_matrix[:, 6, 6], label="var(f) (1/s)^2")
    plt.plot(ts, cov_matrix[:, 7, 7], label="var(b) (1/s)^2")
    plt.xlabel("Time (s)")
    plt.ylabel("Variance of rotational frequencies")
    plt.legend()

    fig = custom_figure("Estimated state of observer (p, e, lambda)")
    plt.plot(ts, rad2deg(xs_estimated_state[:, 0]), label="p observed (°)")
    plt.plot(ts, rad2deg(xs_estimated_state[:, 1]), label="e observed (°)")
    plt.plot(ts, rad2deg(xs_estimated_state[:, 2]), label="lambda observed (°)")
    plt.plot(ts, rad2deg(xs[:, 0]), label="p from simulation (°)")
    plt.plot(ts, rad2deg(xs[:, 1]), label="e from simulation (°)")
    plt.plot(ts, rad2deg(xs[:, 2]), label="lambda from simulation (°)")
    plt.xlabel("Time (s)")
    plt.ylabel("Angle variables")
    plt.legend()

    fig = custom_figure("Estimated state of observer (dp, de, dlambda)")
    plt.plot(ts, rad2deg(xs_estimated_state[:, 3]), label="dp observed (°/s)")
    plt.plot(ts, rad2deg(xs_estimated_state[:, 4]), label="de observed (°/s)")
    plt.plot(ts, rad2deg(xs_estimated_state[:, 5]), label="dlambda observed (°/s)")
    plt.plot(ts, rad2deg(xs[:, 3]), label="dp from simulation (°/s)")
    plt.plot(ts, rad2deg(xs[:, 4]), label="de from simulation (°/s)")
    plt.plot(ts, rad2deg(xs[:, 5]), label="dlambda from simulation (°/s)")
    plt.xlabel("Time (s)")
    plt.ylabel("Angle Velocities")
    plt.legend()

    fig = custom_figure("Estimated state of observer (f, b)")
    plt.plot(ts, xs_estimated_state[:, 6], label="f observed (1/s)")
    plt.plot(ts, xs_estimated_state[:, 7], label="b observed (1/s)")
    plt.plot(ts, xs[:, 6], label="f from simulation (1/s)")
    plt.plot(ts, xs[:, 7], label="b from simulation (1/s)")
    plt.xlabel("Time (s)")
    ...
    plt.plot(ts, rad2deg(xs[:, 0] - xs_estimated_state[:, 0]), label="p - estimated p (°)")
    plt.plot(ts, rad2deg(xs[:, 1] - xs_estimated_state[:, 1]), label="e - estimated e (°)")
    plt.plot(ts, rad2deg(xs[:, 2] - xs_estimated_state[:, 2]), label="lambda - estimated lambda (°)")
    plt.xlabel("Time (s)")
    plt.ylabel("Angle estimate error")
    plt.legend()

    fig = custom_figure("Estimate error of observer (dp, de, dlambda)")
    plt.plot(ts, rad2deg(xs[:, 3] - xs_estimated_state[:, 3]), label="dp - estimated dp (°/s)")
    plt.plot(ts, rad2deg(xs[:, 4] - xs_estimated_state[:, 4]), label="de - estimated de (°/s)")
    plt.plot(ts, rad2deg(xs[:, 5] - xs_estimated_state[:, 5]), label="dlambda - estimated dlambda (°/s)")
    plt.xlabel("Time (s)")
    plt.ylabel("Angle velocity estimate error")
    plt.legend()

    fig = custom_figure("Estimated state of observer (f, b)")
    plt.plot(ts, xs[:, 6] - xs_estimated_state[:, 6], label="f observed (1/s)")
    plt.plot(ts, xs[:, 7] - xs_estimated_state[:, 7], label="b observed (1/s)")
    plt.xlabel("Time (s)")
    ...
    ax1.plot(ts, us_noisy_input[:, 0], label="Vf with noise (V)")
    ax1.plot(ts, us_noisy_input[:, 1], label="Vb with noise (V)")
    ax1.plot(ts, us_ff[:, 0] + us_controller[:, 0], label="Vf without noise (V)")
    ax1.plot(ts, us_ff[:, 1] + us_controller[:, 1], label="Vf without noise (V)")
    plt.xlabel("Time (s)")
    ...
    ax1 = fig.add_subplot(111)
    ax1.plot(ts, rad2deg(ys_noisy_output[:, 0]), label="p with noise (°)")
    ax1.plot(ts, rad2deg(ys_noisy_output[:, 1]), label="e with noise (°)")
    ax1.plot(ts, rad2deg(ys_noisy_output[:, 2]), label="lambda with noise (°)")
    ax1.plot(ts, rad2deg(xs[:, 0]), label="p without noise (°)")
    ax1.plot(ts, rad2deg(xs[:, 1]), label="e without noise (°)")
    ax1.plot(ts, rad2deg(xs[:, 2]), label="lambda without noise (°)")
    plt.xlabel("Time (s)")
    ...
    ax1 = fig.add_subplot(111)
    ax1.plot(ts, ys_noisy_output[:, 3], label="f with noise (1/s)")
    ax1.plot(ts, ys_noisy_output[:, 4], label="b with noise (1/s)")
    ax1.plot(ts, xs[:, 6], label="f without noise (1/s)")
    ax1.plot(ts, xs[:, 7], label="b without noise (1/s)")
    plt.xlabel("Time (s)")
    plt.ylabel("Motor rotational frequencies")
    ax1.legend(loc=2)

...
for i=1:length(t)
    ...
    [A_i, B_i] = TimeVariantLQRI.get_SS_at_time(trajectory, t_i);
    tau_i = tau_e - t_i;
    P_triu_i = interp1(riccati_tau, riccati_P_triu_tau, tau_i);
    P_i = TimeVariantLQRI.triu_to_full(P_triu_i);
    K_i = R_inv * B_i' * P_i;
...
hold on
grid on
plot(t, reshape(K(:, :, 7:9), length(t), 6))
legend('uf iphi', 'ub iphi', 'uf ieps', 'ub ieps', 'uf ilamb', 'ub ilamb')
...
for i=1:n_files
    ...
    sampling_interval = (t(end) - t(1))/(numel(t) - 1);
    ...
    while i_start < numel(t)
        ...
        clips(end+1) = struct('t', (0:(samples_clip-1))*sampling_interval,...
                              ...
                              'ub', ub(i_start:i_end),...
                              'file', file_name,...
                              'tSample', sampling_interval,...
                              'tStart', (i_start-1)*sampling_interval,...
                              'tEnd', i_end*sampling_interval);
...
%% make iddata
output_cell = cell(1, n_clips);
input_cell = cell(1, n_clips);
experiment_names = cell(1, n_clips);
sampling_times = cell(1, n_clips);
...
for i=1:n_clips
    ...
    sampling_times{i} = clips(i).tSample;
end
